  ...
  return (dispatch, getState) => {
    ...
    api.getFacilityData(id, 'diagnoses').then(data => {
      ...
      data.forEach(d => {
        ...
        d.values.forEach(v => {
          // "xLoqtMo0pI";"Umri chini ya mwezi 1"
          ...
          if (v.age.dhis2Id == "GF4Nq9E8x6l") {
            ranges.between5and60 += v.value
          }
          // "UsRGaDRgUTs";"Umri miaka 60 au zaidi"
          if (v.age.dhis2Id == "UsRGaDRgUTs") {
            ranges.greaterThan60 += v.value
          }
      ...
      dispatch({ 'type': FACILITY_DIAGNOSES_RESPONSE, data: parsedData })
    }).catch(error => {
